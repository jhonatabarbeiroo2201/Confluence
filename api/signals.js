...
function analyzePair(symbol, candles) {
  ...
  if (buyScore >= 1 && buyScore >= sellScore) {
    ...
    let alvo;
    if (buyScore === 3) {
      // confluência forte (tendência + suporte/resistência + Fibonacci todos alinhados) — vale deixar correr mais
      alvo = resistances[1] !== undefined ? resistances[1] : currentPrice + range * 0.55;
    } else {
      alvo = nearestResistance !== undefined ? nearestResistance : currentPrice + range * 0.3;
    }
  ...
  if (sellScore >= 1) {
    ...
    let alvo;
    if (sellScore === 3) {
      alvo = supports[1] !== undefined ? supports[1] : currentPrice - range * 0.55;
    } else {
      alvo = nearestSupport !== undefined ? nearestSupport : currentPrice - range * 0.3;
    }
