...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'login') {
      ...
      const token = randomToken();
      await redis(['SET', `confluence:session:${token}`, username]);
      res.status(200).json({ ok: true, token, username, email: rec.email, role: rec.role, status: rec.status, validUntil: rec.validUntil });
      return;
    }
    ...
    if (action === 'session') {
      ...
      res.status(200).json({ ok: true, username, email: rec.email, role: rec.role, status: rec.status, validUntil: rec.validUntil });
      return;
    }
    ...
    if (action === 'signup') {
      ...
      const token = randomToken();
      await redis(['SET', `confluence:session:${token}`, username]);
      res.status(200).json({ ok: true, token, username, email, role: 'subscriber', status: 'blocked', validUntil: users[username].validUntil });
      return;
    }

    if (['admin-list', 'admin-delete-user', 'admin-kiwify-debug'].includes(action)) {
      const { token } = body;
      ...
      if (action === 'admin-kiwify-debug') {
        const raw = await redis(['GET', 'confluence:last_kiwify_payload']);
        res.status(200).json({ ok: true, payload: raw ? JSON.parse(raw) : null });
        return;
      }
    }
