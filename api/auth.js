...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'create-test-subscriber') {
      const users = await getUsers();
      const uname = 'assinante' + Math.floor(Math.random() * 9000 + 1000);
      const plainPw = 'teste123';
      const passwordHash = crypto.createHash('sha256').update(plainPw).digest('hex');
      users[uname] = { passwordHash, role: 'subscriber', status: 'active', validUntil: addDays(30), createdAt: todayStr() };
      await saveUsers(users);
      const token = randomToken();
      await redis(['SET', `confluence:session:${token}`, uname]);
      res.status(200).json({ ok: true, token, username: uname, password: plainPw, role: 'subscriber', status: 'active', validUntil: users[uname].validUntil });
      return;
    }

    if (['admin-list', 'admin-set-status', 'admin-set-validity'].includes(action)) {
      const { token } = body;
      ...
      if (action === 'admin-set-status') {
        const { targetUser, status } = body;
        if (users[targetUser]) { users[targetUser].status = status; await saveUsers(users); }
        res.status(200).json({ ok: true }); return;
      }
      if (action === 'admin-set-validity') {
        const { targetUser, validUntil } = body;
        if (users[targetUser]) { users[targetUser].validUntil = validUntil; users[targetUser].status = 'active'; await saveUsers(users); }
        res.status(200).json({ ok: true }); return;
      }
    }
